...
# モデル全体をロード
for file_number_dot in range(3, 4):
        ...
        pinns = torch.load(f'model_spininit.pth', map_location=torch.device('cuda'))
...
######################
                ...
                # plot e
                data          = e_plot
                vmax_real     = 1.025 # np.max(data) # for value range
              ...
              # plot ur
                data          = ur_plot
                vmax_real     = 0.006 # np.max(data) # for color range
              ...
              # plot uth
                ...
                vmin_real     =-0.6  # np.min(data) # for color range
                vmax_sym      = max(abs(vmin_real), abs(vmax_real))
#                norm          = Normalize(vmin=-vmax_sym, vmax=vmax_sym)
...
######################################################
                ...
                if print_dat ==1:
                        ...
                        np.savetxt("Ltot_spininit_v2.dat",np.column_stack((t_values, np.array(l_total_values) + np.array(s_total_values), l_total_values, s_total_values)),header="Time t, Angular Momentum",fmt="%.6e")
######################################################
######################################################
